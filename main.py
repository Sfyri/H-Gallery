...
@app.middleware("http")
async def _m6_limit_lan_access(request: _M6Request, call_next):
    client_host = request.client.host if request.client else ""
    is_loopback = client_host in {"127.0.0.1", "::1"} or client_host.startswith("::ffff:127.")
    path = request.url.path
    is_mobile_sync = path.startswith("/api/mobile/sync/")
    if not is_loopback and path not in _M6_REMOTE_ROUTES and not is_mobile_sync:
        return _M6JSONResponse(
            status_code=403,
            content={"detail": "Accesso LAN consentito solo al collegamento H-Gallery Android."},
        )
    return await call_next(request)


@app.get("/mobile", include_in_schema=False)
def _m6_pairing_page():
    return FileResponse(FRONTEND_ROOT / "mobile_pairing.html")


@app.get("/api/mobile/hello")
def _m6_mobile_hello():
    return _M6_MOBILE_LINK.hello()


@app.post("/api/mobile/pair")
def _m6_mobile_pair(request: _M6PairRequest):
    try:
        return _M6_MOBILE_LINK.pair(
...
def _m6_mobile_status(request: _M6StatusRequest):
    try:
        return _M6_MOBILE_LINK.status(device_id=request.device_id, token=request.token)
    except PermissionError as error:
        raise HTTPException(status_code=401, detail=str(error)) from error


@app.get("/api/mobile/pairing")
def _m6_pairing_status():
    return _M6_MOBILE_LINK.pairing_status()


@app.post("/api/mobile/pairing/refresh")
def _m6_refresh_pairing():
    return _M6_MOBILE_LINK.refresh_pairing_code()


def _m7_authorize(device_id: str, token: str, *, touch: bool = False):
    try:
        return _M6_MOBILE_LINK.authorize(device_id=device_id, token=token, touch=touch)
    except PermissionError as error:
        raise HTTPException(status_code=401, detail=str(error)) from error
...
def _m7_begin(request: _M7SyncRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Backup pre-sync non riuscito: {error}") from error


@app.get("/api/mobile/sync/file/{sync_uuid}")
def _m7_file(
    sync_uuid: str,
...
@app.post("/api/mobile/sync/upload")
async def _m7_upload(
    request: _M6Request,
    x_hgallery_device: str = _M7Header(alias="X-HGallery-Device"),
    x_hgallery_token: str = _M7Header(alias="X-HGallery-Token"),
    x_hgallery_group: str = _M7Header(alias="X-HGallery-Group"),
    ...
    try:
        ...
        if not isinstance(media, dict):
            raise ValueError("Metadati upload non validi.")
    except (ValueError, UnicodeError, _M7json.JSONDecodeError) as error:
        raise HTTPException(status_code=400, detail="Metadati upload non validi.") from error

    temporary = _M7_MOBILE_SYNC.new_temporary_upload()
    try:
        with temporary.open("wb") as handle:
            async for chunk in request.stream():
                if chunk:
                    handle.write(chunk)
        return _M7_MOBILE_SYNC.import_uploaded_file(temporary, media)
