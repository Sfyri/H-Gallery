...
class _GalleryMergePageState extends State<GalleryMergePage> {
  ...
  Future<void> _analyze() async {
    ...
    try {
      final plan = await _syncService.analyze(widget.androidGallery, widget.windowsGallery, widget.syncGroupUuid);
  ...
  Future<void> _sync() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          ...
          'Il merge è additivo: i file con lo stesso SHA-256 non saranno duplicati e le eliminazioni non vengono propagate.',
        ),
        actions: [
          TextButton(onPressed: () => Navigator.of(context).pop(false), child: const Text('Annulla')),
          FilledButton.icon(
    ...
    setState(() {
      _syncing = true;
      _result = null;
      _progress = const GallerySyncProgress(phase: 'start', processed: 0, total: 1, current: 'Preparazione sincronizzazione');
    });
    try {
      final result = await _syncService.run(widget.androidGallery, widget.windowsGallery, widget.syncGroupUuid);
      if (!mounted) return;
      setState(() => _result = result);
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Merge M7 completato.')));
      final refreshed = await _syncService.analyze(widget.androidGallery, widget.windowsGallery, widget.syncGroupUuid);
      if (mounted) setState(() => _plan = refreshed);
    } on PlatformException catch (error) {
      _error(error.message ?? 'Sincronizzazione non riuscita. Puoi rilanciarla: i file già completati verranno riconosciuti dall’hash.');
    } finally {
  ...
  String _phaseLabel(GallerySyncProgress progress) {
    switch (progress.phase) {
      case 'scan': return 'Indicizzazione';
      case 'compare': return 'Confronto gallerie';
      case 'download': return 'Windows → Android';
      case 'upload': return 'Android → Windows';
      case 'metadata_android': return 'Merge metadata Android';
      case 'metadata_windows': return 'Merge metadata Windows';
      case 'finalize_windows': return 'Aggiornamento Windows';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          const Text('Merge bidirezionale', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800)),
          const SizedBox(height: 12),
          Card(
            child: Padding(
              padding: const EdgeInsets.all(18),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Android · ${widget.androidGallery.name}', style: const TextStyle(fontWeight: FontWeight.w700)),
                  const SizedBox(height: 5),
                  Text('Windows · ${widget.windowsGallery.name}', style: const TextStyle(fontWeight: FontWeight.w700)),
                  const SizedBox(height: 8),
                  Text('Gruppo ${widget.syncGroupUuid}', style: const TextStyle(color: AppTheme.muted, fontSize: 11)),
          ...
          if (plan != null) ...[
            ...
            if (plan.pathConflicts > 0) ...[
              ...
              Card(
                child: Padding(
                  ...
                  child: Text('${plan.pathConflicts} collisioni di percorso: entrambe le versioni saranno conservate.', style: const TextStyle(height: 1.4)),
                ),
              ),
            ],
            const SizedBox(height: 14),
            FilledButton.icon(
              ...
              icon: const Icon(Icons.sync_rounded),
              label: Text(plan.toAndroid == 0 && plan.toWindows == 0 ? 'Sincronizza metadata' : 'Sincronizza gallerie'),
            ),
          ],
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (progress.total > 1) Text('${progress.processed}/${progress.total}', style: const TextStyle(color: AppTheme.muted, fontSize: 12)),
                  ],
                ),
              ),
            ),
            ...
            Card(
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Ultimo merge completato', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16)),
                    const SizedBox(height: 12),
